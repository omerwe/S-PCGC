...
The following is a simple end-to-end S-PCGC analysis. We will estimate heritability, genetic correlation and functional enrichment for two simulated case-control studies of diseases with a population prevalence of 1%, found in the directory `example`. To begin, please cd into the S-PCGC directory, and type the following commands (using the anaconda version of python if available):
```
mkdir temp_results
