...
A: Not necessarily. S-PCGC first estimates annotation effects via a subset of SNPs, and then estimates heritability and enrichment using all common SNPs that appear in the reference panel. The subset of SNPs corresponds to the set of "regression SNPs" of S-LDSC. We recommend using a set of reliable common SNPs that are probably either genotyped or well-imputed, such as HapMap3 SNPs. We also recommend excluding SNPs within the MHC (chromosome 6 28M-32M) from all analyses, as done by S-LDSC and other tools.
