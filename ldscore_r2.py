...
class __GenotypeArrayInMemory__(object):
    ...
    def compute_r2prod(self, r2_arr, A_x, A_y):
        r2_A_x = np.tile(np.sqrt(np.abs(r2_arr)) * np.sign(r2_arr), (A_x.shape[1], 1, 1)) * A_x.T[:,:,np.newaxis]**2
        r2_A_y = np.tile(np.sqrt(np.abs(r2_arr)), (A_x.shape[1], 1, 1)) * A_y.T[:, np.newaxis, :]**2
        r2_table = np.tensordot(r2_A_x, r2_A_y, [(1,2),(1,2)])
        return r2_table
